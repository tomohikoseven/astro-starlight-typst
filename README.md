...
Astro のドキュメントフレームワーク **Starlight** と、モダンな組版システム **Typst** を組み合わせたブログ/ドキュメントサイトです。
...
- **CeTZ による図式描画**: Typst の強力な描画ライブラリ **CeTZ 0.5.2** を標準サポート。複雑な幾何学図形やグラフ、ダイアグラムをコードで美しく描画できます。
- **Fletcher による高度な図式描画**: 可換図式やフローチャート、状態遷移図を数式モード感覚で美しく直感的に描画できる **Fletcher 0.5.8** を追加サポート。
- **Physica による物理・数学記法**: 偏微分、ディラックのブラケット記法、ベクトル場、行列などの高度な数式表記を劇的に簡略化する **Physica** を追加サポート。
